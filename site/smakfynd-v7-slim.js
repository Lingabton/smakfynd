...
function Card({
  ...
  const comparison = useMemo(() => {
    ...
    const candidates = allProducts.filter(w => {
      ...
      const similarTaste = myBody && w.taste_body && Math.abs(w.taste_body - myBody) <= 2 && (!myFruit || !w.taste_fruit || Math.abs(w.taste_fruit - myFruit) <= 3);
...
function StoreMode({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    }).filter(w => w._sim >= 8).sort((a, b) => b._sim - a._sim || b.smakfynd_score - a.smakfynd_score).slice(0, 3);
    if (similar.length === 0) return null;
