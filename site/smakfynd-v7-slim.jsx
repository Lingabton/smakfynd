...
function _sendSessionEnd() {
  ...
  const payload = JSON.stringify({
    session: _sid, event: "session_end",
    data: JSON.stringify({ duration_s }),
    page: location.hash || "/", device, referrer: document.referrer,
  });
  if (navigator.sendBeacon) {
    navigator.sendBeacon(ANALYTICS_URL + "/event", new Blob([payload], { type: "application/json" }));
  } else {
    fetch(ANALYTICS_URL + "/event", { method: "POST", headers: {"Content-Type": "application/json"}, body: payload, keepalive: true }).catch(() => {});
  }
}
let _sessionEnded = false;
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden" && !_sessionEnded) { _sessionEnded = true; _sendSessionEnd(); }
});
window.addEventListener("pagehide", () => {
  if (!_sessionEnded) { _sessionEnded = true; _sendSessionEnd(); }
});
