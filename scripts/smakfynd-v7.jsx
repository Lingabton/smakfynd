    ...
    >
      ...
      <div style={{ padding: "16px 18px", display: "flex", gap: 14, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* Row 4: Why this wine ranks high */}
          <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}>
            {p.crowd_score >= 8.0 && <span style={{ fontSize: 9, color: "#6b8cce", background: "#6b8cce10", padding: "2px 7px", borderRadius: 100 }}>Starkt crowd-stöd</span>}
            {p.crowd_score >= 7.0 && p.crowd_score < 8.0 && p.crowd_reviews >= 10000 && <span style={{ fontSize: 9, color: "#6b8cce", background: "#6b8cce10", padding: "2px 7px", borderRadius: 100 }}>Många omdömen</span>}
            {p.expert_score >= 7.5 && <span style={{ fontSize: 9, color: "#b07d3b", background: "#b07d3b10", padding: "2px 7px", borderRadius: 100 }}>Gott expertstöd</span>}
            {p.price_score >= 8 && <span style={{ fontSize: 9, color: t.green, background: `${t.green}10`, padding: "2px 7px", borderRadius: 100 }}>Prisvärt i sin kategori</span>}
            {p.price_score >= 6 && p.price_score < 8 && p.crowd_score >= 7.5 && <span style={{ fontSize: 9, color: t.txM, background: `${t.txM}10`, padding: "2px 7px", borderRadius: 100 }}>Bra betyg till rimligt pris</span>}
          </div>
        </div>
      </div>
...
function StoreMode({ products, onClose }) {
  ...
  const result = useMemo(() => {
    ...
    if (!match) return { match: null, alternatives: [] };
    ...
    return { match, alternatives: alts };
  }, [q, products]);
    ...
    <div style={{
      ...
      <div style={{ position: "relative", marginBottom: 20 }}>
        <input ref={inputRef} type="search" inputMode="search" value={q} onChange={e => setQ(e.target.value)}
          placeholder="Vinnamn eller artikelnr (från hyllkanten)..."
          style={{
            width: "100%", padding: "18px 20px 18px 48px", borderRadius: 16,
            border: `2px solid ${t.wine}30`, background: t.card, fontSize: 18,
        ...
        />
        <span style={{ position: "absolute", left: 18, top: "50%", transform: "translateY(-50%)", fontSize: 20, color: t.txL, pointerEvents: "none" }}>🔍</span>
      </div>
      ...
      <div style={{ maxWidth: 580, margin: "0 auto", padding: "24px 16px 80px" }}>
        ...
        <div style={{ marginBottom: 14, padding: "0 4px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            ...
            <span style={{ fontSize: 11, color: t.txF }}>Sorterat efter kvalitet per krona</span>
          </div>
          <div style={{ fontSize: 10, color: t.txF, marginTop: 3 }}>Poängen visar prisvärde — inte "bästa vinet", utan mest smak för pengarna i sin kategori.</div>
        </div>
